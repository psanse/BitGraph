...
namespace bitgraph {
	...
	// 
	// @brief T = Ugraph<BBScanSp> with alias facade type sparse_bitarray
	//
	////////////////////////
	...
	template<>
	inline
	int Ugraph<BBScanSp>::degree_up(int v) const
	{
	...
	template<>
	inline
	int Ugraph<BBScanSp>::degree(int v) const 
	{
	...
	template<>
	inline
	int Ugraph<BBScanSp>::degree(int v, const Bitset& bbn) const
	{
		int ndeg = 0;
	...
	template<>
	inline
	int Ugraph<BBScanSp>::degree(int v, const BitsetSp& bbs) const 
	{
		int ndeg = 0;
	...
	template<>
	inline
	int Ugraph<BBScanSp>::degree(int v, int UB, const Bitset& bbn) const
	{
		int ndeg = 0;
		for (auto it = adj_[v].cbegin(); it != adj_[v].cend(); ++it) {
			ndeg += bblock::popc64(it->bb_ & bbn.block(it->idx_));
	...
	template<>
	inline
	int Ugraph<BBScanSp>::degree(int v, int UB, const BitsetSp& bbs) const
	{
		int ndeg = 0;
		auto itv = adj_[v].cbegin();
		auto itbb = bbs.cbegin();
	...
	template<>
	inline
	void Ugraph<BBScanSp>::write_dimacs(ostream& o) 
	{
	...
	template<>
	inline
	std::ostream& Ugraph<BBScanSp>::print_edges(std::ostream& o, bool eofl)
	{
		for (int v = 0; v < this->NV_ - 1; ++v) {
	...
	template<>
	inline
	void  Ugraph<BBScanSp>::write_EDGES(ostream& o)
	{
	...
	template<>
	inline
	void  Ugraph<BBScanSp>::write_mtx(ostream& o) 
	{
		// writes simple unweighted grafs  in edge list format 
		// note: loops are not allowed
...
#endif // end __SIMPLE_SPARSE_UGRAPH_H__